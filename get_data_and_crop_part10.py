...
bad_img = ["drescher5.jpg", "drescher122.jpg", "ferrera165.jpg"]
...
def check_hash(filename, sha256):
    file = open("uncropped_p10/"+filename, "rb")
    m = hashlib.sha256()
    m.update(file.read())
    print(m.hexdigest())
    print(sha256)
    ...
    if (m.hexdigest() != sha256):
        print("sha256 check fails")
        os.remove("uncropped_p10/"+filename)
...
def get_data_and_crop():
    for a in act:
        ...
        i = 0
        for line in open("facescrub_actors.txt"): #save
            if a in line:
                
                filename = name+str(i)+'.'+line.split()[4].split('.')[-1]
                bound_box = line.split()[5].split(',')
                timeout(testfile.retrieve, (line.split()[4], "uncropped_p10/"+filename), {}, 30)
                
                if not os.path.isfile("uncropped_p10/"+filename):
                ...
                # Removes bad images with sha256.
                sha256 = line.split()[6]
                check_hash(filename, sha256)
                
                # Some uncropped images are broken/fails sha256 check/low quality image and cannot be processed.
                try:
                    img = imread('uncropped_p10/'+filename)
                    img = img[y1:y2,x1:x2]
                    #img = rgb2gray(img)
                    img = imresize(img, (227, 227))
                    imsave('cropped_p10/' + filename, img)
                    print(filename)
                except:
                    print("Couldn't process the file: "+filename)
                print("---------------------------------------------------")
...
if not os.path.exists("cropped_p10/"):
    os.makedirs("cropped_p10/")
    
if not os.path.exists("uncropped_p10/"):
    os.makedirs("uncropped_p10/")
